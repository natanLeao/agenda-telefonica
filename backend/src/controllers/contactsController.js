...
exports.getContacts('/contatos/', async (req, res) => {

  try {
    const [rows] = await pool.query('SELECT * FROM contatos');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: 'Erro ao buscar contatos', error});
 }
});

// [POST] Adicionar um contato
exports.addContact('/contatos/', async (req, res) => {
...
exports.deleteContact('/contatos/id/', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao excluir contato', error});
  }
});
